...
PRINT_TO_FILE="true"
VALGRIND="false"
# CACHE_SIZES=(512 1024 2048 4096 8192 16384 32768 65536 131072 262144 524288 1048576 2097152 4194304)
# CACHE_SIZES=(16384 32768 262144 524288 1048576)
CACHE_SIZES=(262144)

BIN_SCALES=(16 16 8 4 2)
MODES=("orig")
BIN_CONFIG_PATH="/localhome/mha157/gem5-SALAM/src/hwacc/bin_config.txt"
# PY_FILE_PATH="/localhome/mha157/gem5-SALAM/4.txt"
...
for mode in ${MODES[@]}; do
	for t in ${CACHE_SIZES[@]}; do
		...
		if [ "${PRINT_TO_FILE}" == "true" ]; then
			mkdir -p $OUTDIR
			$RUN_SCRIPT > ${OUTDIR}/debug-trace.txt > 3.txt 2> 4.txt
		else
			$RUN_SCRIPT  > 3.txt 2> 4.txt
		fi
		echo "Done with cache size: ${t}"
		cp 3.txt $OUTDIR/SALAM_OUT_${mode}_${t}.txt
		cp 4.txt $OUTDIR/log_${mode}_${t}.txt
