...
bool ScratchpadMemory::isReady(Addr ad, size_t size, bool read) {
  if (!readyMode) {
    return true;
  } else if (read) {
...
void ScratchpadMemory::setAllReady(bool r) {
...
void ScratchpadMemory::scratchpadAccess(PacketPtr pkt, bool validateAccess) {
  initial = false;
  ...
  if (pkt->cmd == MemCmd::SwapReq) {
    ...
    if (validateAccess) {
      if (!isReady(pkt->getAddr(), pkt->getSize(), true)) {
        panic(
            "Scratchpad read at address: 0x%lx is invalid! Sector has not been "
            "written yet!\n",
            pkt->getAddr());
      }
    ...
    if (validateAccess) {
      if (!isReady(pkt->getAddr(), pkt->getSize(), false)) {
        panic(
            "Scratchpad write at address: 0x%lx is invalid! Sector has not "
            "been cleared yet!\n",
            pkt->getAddr());
      }
    }
...
Tick ScratchpadMemory::recvAtomic(PacketPtr pkt, bool validateAccess) {
...
Tick ScratchpadMemory::recvAtomicBackdoor(PacketPtr pkt,
                                          MemBackdoorPtr &_backdoor) {
  Tick latency = recvAtomic(pkt);

  if (backdoor.ptr()) _backdoor = &backdoor;
  return latency;
}

void ScratchpadMemory::recvFunctional(PacketPtr pkt) {
  pkt->pushLabel(name());
...
bool ScratchpadMemory::recvTimingReq(PacketPtr pkt, PortID recvPort,
                                     bool validateAccess) {
  panic_if(pkt->cacheResponding(),
           "Should not see packets where cache "
           "is responding");
...
Tick ScratchpadMemory::getLatency() const {
  return latency + (latency_var ? random_mt.random<Tick>(0, latency_var) : 0);
}

void ScratchpadMemory::recvRespRetry(PortID id) {
  PortID idx = id + 1;
...
Port &ScratchpadMemory::getPort(const std::string &if_name, PortID idx) {
  if (if_name == "port") {
    return port;
  } else if (if_name == "spm_ports") {
    if (idx >= spm_ports.size()) {
      spm_ports.resize((idx + 1), nullptr);
      // const std::string releaseEventName = csprintf("%s_release[%d]", name(),
      // (idx+1));
      const std::string releaseEventName =
          name() + "_release[" + std::to_string(idx + 1) + "]";
      releaseEvent.resize((idx + 2), EventFunctionWrapper([this] { release(); },
                                                          releaseEventName));
...
DrainState ScratchpadMemory::drain() {
  if (!packetQueue.empty()) {
...
AddrRangeList ScratchpadMemory::MemoryPort::getAddrRanges() const {
  AddrRangeList ranges;
  ranges.push_back(memory.getAddrRange());
  return ranges;
}

Tick ScratchpadMemory::MemoryPort::recvAtomic(PacketPtr pkt) {
  return memory.recvAtomic(pkt);
}

Tick ScratchpadMemory::MemoryPort::recvAtomicBackdoor(
    PacketPtr pkt, MemBackdoorPtr &_backdoor) {
  return memory.recvAtomicBackdoor(pkt, _backdoor);
}

void ScratchpadMemory::MemoryPort::recvFunctional(PacketPtr pkt) {
  memory.recvFunctional(pkt);
}

bool ScratchpadMemory::MemoryPort::recvTimingReq(PacketPtr pkt) {
  return memory.recvTimingReq(pkt, id);
}

void ScratchpadMemory::MemoryPort::recvRespRetry() { memory.recvRespRetry(id); }
