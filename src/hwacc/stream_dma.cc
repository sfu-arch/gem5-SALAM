...
StreamDma::StreamDma(const StreamDmaParams &p)
  ...
  for (int i = 0; i < pioSize; i++) mmreg[i] = 0;
  FLAGS = mmreg;
...
AddrRangeList StreamDma::getAddrRanges() const {
  assert(pioSize != 0);
...
AddrRangeList StreamDma::getStreamAddrRanges() const {
  assert(streamSize != 0);
...
AddrRangeList StreamDma::getStatusAddrRanges() const {
  assert(statusSize != 0);
...
void StreamDma::tick() {
  if (!rdRunning && ((*FLAGS & RD_START_MASK) == RD_START_MASK)) {
    ...
    *FLAGS |= RD_RUNNING_MASK;
    ...
    DPRINTF(
        StreamDma,
        "Initializing frame read from 0x%016x with frame size of %d Bytes\n",
  ...
  if (!wrRunning && ((*FLAGS & WR_START_MASK) == WR_START_MASK)) {
    ...
    *FLAGS |= WR_RUNNING_MASK;
    ...
    DPRINTF(StreamDma,
            "Initializing frame write to 0x%016x with frame size of %d Bytes\n",
  ...
  if (wrRunning && !writeFifo->isActive()) {
    ...
    if ((framesToWrite != 0) && (framesWritten >= framesToWrite)) {
      ...
      *FLAGS &= ~WR_RUNNING_MASK;
      ...
      writePtr =
          writeAddr + ((framesWritten % writeFrameBuffSize) * writeFrameSize);
      DPRINTF(
          StreamDma,
          "Initializing frame write to 0x%016x with frame size of %d Bytes\n",
          writePtr, writeFrameSize);
...
Tick StreamDma::read(PacketPtr pkt) {
...
Tick StreamDma::write(PacketPtr pkt) {
  Addr offset = pkt->req->getPaddr() - pioAddr;
...
Tick StreamDma::streamRead(PacketPtr pkt) {
  DPRINTF(DeviceMMR,
          "The data buffer associated with this DMA was read from!\n");
...
Tick StreamDma::streamWrite(PacketPtr pkt) {
  DPRINTF(DeviceMMR,
          "The data buffer associated with this DMA was written to!\n");
  uint8_t *data = new uint8_t[pkt->getSize()];
  pkt->writeData(data);
...
Tick StreamDma::status(PacketPtr pkt, bool readStatus) {
  ...
  if (pkt->isRead()) {
    ...
    if (readStatus) {
      DPRINTF(StreamDma,
              "The status of the MM2S buffer has been read. Current capacity "
              "is %d of %d bytes\n",
              readFifo->size(), rdBufferSize);
      data = readFifo->size();
    } else {
      DPRINTF(StreamDma,
              "The status of the S2MM buffer has been read. Current capacity "
              "is %d of %d bytes\n",
...
bool StreamDma::tvalid(PacketPtr pkt) {
  return tvalid(pkt->getSize(), pkt->isRead());
}

bool StreamDma::tvalid(size_t len, bool isRead) {
  if (isRead) {
...
Port &StreamDma::getPort(const std::string &if_name, PortID idx) {
