...
NoncoherentDma::NoncoherentDma(const NoncoherentDmaParams &p)
  ...
  memSideReadFifo =
      new DmaReadFifo(dmaPort, size_t(bufferSize / 2), maxReqSize, maxPending);
  memSideWriteFifo =
      new DmaWriteFifo(dmaPort, size_t(bufferSize / 2), maxReqSize, maxPending);
  accSideReadFifo =
      new DmaReadFifo(accPort, size_t(bufferSize / 2), maxReqSize, maxPending);
  accSideWriteFifo =
      new DmaWriteFifo(accPort, size_t(bufferSize / 2), maxReqSize, maxPending);
  readFifo = nullptr;
  writeFifo = nullptr;
...
AddrRangeList NoncoherentDma::getAddrRanges() const {
  assert(pioSize != 0);
...
DmaReadFifo *NoncoherentDma::getActiveReadFifo() {
...
DmaWriteFifo *NoncoherentDma::getActiveWriteFifo() {
  AddrRangeList accPortRanges = accPort.getAddrRanges();
  for (auto range : accPortRanges) {
...
Tick NoncoherentDma::read(PacketPtr pkt) {
...
Tick NoncoherentDma::write(PacketPtr pkt) {
  DPRINTF(DeviceMMR,
...
Port &NoncoherentDma::getPort(const std::string &if_name, PortID idx) {
