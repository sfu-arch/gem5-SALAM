...
bool IOAcc::MemSidePort::recvTimingResp(PacketPtr pkt) {
  owner->recvPacket(pkt);
  return true;
}

void IOAcc::MemSidePort::recvReqRetry() {
...
void IOAcc::MemSidePort::sendPacket(PacketPtr pkt) {
...
void IOAcc::recvPacket(PacketPtr pkt) {
  if (pkt->isRead()) {
...
void IOAcc::tryRead() {
  ...
  if (readLeft <= 0) {
    DPRINTF(IOAcc,
            "Something went wrong. Shouldn't try to read if there aren't reads "
            "left\n");
    return;
  }
...
void IOAcc::tryWrite() {
  ...
  RequestPtr req =
      make_shared<Request>(currentWriteAddr, size, flags, masterId);
  ...
  DPRINTF(IOAcc, "Trying to write to addr: 0x%x, %d bytes, data 0x%08x\n",
          ...
          *((int *)(&curData[totalLength - writeLeft])));
...
int IOAcc::prepRead(Addr src, size_t length) {
  assert(length > 0);
  assert(!running);
  running = true;
...
int IOAcc::prepWrite(Addr dst, uint8_t *value, size_t length) {
  assert(!running);
  assert(length > 0);
  running = true;
  gic->clearInt(int_num);
...
Tick IOAcc::write(PacketPtr pkt) {
  DPRINTF(IOAcc,
...
IOAcc *IOAccParams::create() { return new IOAcc(this); }

Port &IOAcc::getPort(const std::string &if_name, PortID idx) {
  if (if_name == "mem_side") {
    return memPort;
  } else {
