...
DmaWriteFifo::DmaWriteFifo(DmaPort &_port, size_t size, unsigned max_req_size,
  ...
  for (auto &e : freeRequests) e.reset(new DmaDoneEvent(this, max_req_size));
}
...
bool DmaWriteFifo::canFill(size_t len) {
...
bool DmaWriteFifo::tryFill(uint8_t *src, size_t len) {
  if ((buffer.size() + len) <= fifoSize) {
...
void DmaWriteFifo::fill(uint8_t *src, size_t len) {
  const bool success(tryFill(src, len));
  panic_if(!success, "Buffer overrun in DmaWriteFifo::fill()\n");
}

void DmaWriteFifo::startEmpty(Addr start, size_t size) {
  assert(atEndOfBlock());
...
void DmaWriteFifo::resumeEmpty() {
  ...
  if (!old_eob && atEndOfBlock()) onEndOfBlock();
}

void DmaWriteFifo::resumeEmptyFunctional() {
...
void DmaWriteFifo::resumeEmptyTiming() {
  size_t size_pending(0);
  for (auto &e : pendingRequests) size_pending += e->requestSize();

  while (!freeRequests.empty() && !atEndOfBlock()) {
    const size_t req_size(std::min(maxReqSize, endAddr - nextAddr));
    if ((int64_t)(buffer.size() - size_pending - req_size) < 0) break;
...
void DmaWriteFifo::handlePending() {
  while (!pendingRequests.empty() && pendingRequests.front()->done()) {
    ...
    if (event->canceled()) buffer.write(event->data(), event->requestSize());

    // Move the event to the list of free requests
    freeRequests.emplace_back(std::move(event));
  }

  if (pendingRequests.empty()) signalDrainDone();
}
...
DmaWriteFifo::DmaDoneEvent::DmaDoneEvent(DmaWriteFifo *_parent, size_t max_size)
...
void DmaWriteFifo::DmaDoneEvent::cancel() { _canceled = true; }

void DmaWriteFifo::DmaDoneEvent::reset(size_t size) {
