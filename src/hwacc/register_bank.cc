...
void RegisterBank::registerAccess(PacketPtr pkt) {
  ...
  if (pkt->isRead()) {
    ...
    if (writeOK(pkt)) {
      ...
      if (!deltaEvent.scheduled()) schedule(deltaEvent, curTick() + deltaTime);
    }
  } else {
...
Tick RegisterBank::recvAtomic(PacketPtr pkt) {
...
Tick RegisterBank::recvAtomicBackdoor(PacketPtr pkt,
...
void RegisterBank::recvFunctional(PacketPtr pkt) {
  pkt->pushLabel(name());
...
bool RegisterBank::recvTimingReq(PacketPtr pkt) {
  panic_if(pkt->cacheResponding(),
           "Should not see packets where cache "
           "is responding");
...
void RegisterBank::delta() { std::memcpy(pmemAddr, deltaAddr, range.size()); }

void RegisterBank::recvRespRetry() {
  assert(retryResp);
  dequeue();
}

Port &RegisterBank::getPort(const std::string &if_name, PortID idx) {
  if (if_name == "reg_port") {
    return port;
