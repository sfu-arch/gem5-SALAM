 ...
 *accelerators. It provides a set of memory-mapped registers, as well as master
 *ports for accessing both local busses/SPMs and system memory.
 **************************************************************************************/
...
bool CommInterface::MemSidePort::recvTimingResp(PacketPtr pkt) {
  owner->recvPacket(pkt);
  return true;
}

void CommInterface::MemSidePort::recvReqRetry() {
...
void CommInterface::MemSidePort::sendPacket(PacketPtr pkt) {
...
bool CommInterface::SPMPort::recvTimingResp(PacketPtr pkt) {
  owner->recvPacket(pkt);
  return true;
}

void CommInterface::SPMPort::recvReqRetry() {
  assert(outstandingPkts.size());
...
void CommInterface::SPMPort::sendPacket(PacketPtr pkt) {
  if (isStalled() || !sendTimingReq(pkt)) {
    if (debug())
      DPRINTF(CommInterface,
              "sendTiming failed in sendPacket(pkt->req->getPaddr()=0x%x)\n",
...
bool CommInterface::RegPort::recvTimingResp(PacketPtr pkt) {
  owner->recvPacket(pkt);
  return true;
}

void CommInterface::RegPort::recvReqRetry() {
  panic("We should not be receiving retries from Register Banks.\n");
}

void CommInterface::RegPort::sendPacket(PacketPtr pkt) { sendTimingReq(pkt); }

void CommInterface::recvPacket(PacketPtr pkt) {
  if (pkt->isRead()) {
    ...
    if (!readReq->needToRead) {
      ...
      clearMemRequest(readReq,
                      true);  // Clear the request from the in-flight queue
    }
  } else if (pkt->isWrite()) {
    ...
    if (!(writeReq->needToWrite)) {
      ...
      clearMemRequest(writeReq,
                      false);  // Clear the request from the in-flight queue
    }
  } else {
...
bool CommInterface::inStreamRange(Addr add) {
...
bool CommInterface::inSPMRange(Addr add) {
...
bool CommInterface::inRegRange(Addr add) {
...
bool CommInterface::inLocalRange(Addr add) {
...
bool CommInterface::inGlobalRange(Addr add) {
...
CommInterface::MemSidePort *CommInterface::getValidLocalPort(Addr add,
                                                             bool read) {
  for (auto port : localPorts) {
    AddrRangeList adl = port->getAddrRanges();
    for (auto address : adl) {
...
CommInterface::MemSidePort *CommInterface::getValidGlobalPort(Addr add,
                                                              bool read) {
  for (auto port : globalPorts) {
    AddrRangeList adl = port->getAddrRanges();
    for (auto address : adl) {
...
CommInterface::MemSidePort *CommInterface::getValidStreamPort(Addr add,
                                                              size_t len,
                                                              bool read) {
  for (auto port : streamPorts) {
    AddrRangeList adl = port->getAddrRanges();
...
CommInterface::SPMPort *CommInterface::getValidSPMPort(Addr add, size_t len,
                                                       bool read) {
  for (auto port : spmPorts) {
    AddrRangeList adl = port->getAddrRanges();
    for (auto address : adl) {
      if (address.contains(add) && !(port->isStalled())) {
        if (((read && !(port->readReq)) || (!read && !(port->writeReq)))) {
          if (port->canAccess(add, len, read)) return port;
        }
      }
    }
  }
...
CommInterface::RegPort *CommInterface::getValidRegPort(Addr add) {
  for (auto port : regPorts) {
    AddrRangeList adl = port->getAddrRanges();
    for (auto address : adl) {
      if (address.contains(add)) {
...
void CommInterface::processMemoryRequests() {
  if (!allPortsStalled()) {
    if (debug())
...
void CommInterface::tick() {
  if (debug()) DPRINTF(CommInterface, "Tick!\n");
  checkMMR();
  requestsInQueues = readQueue.size() + writeQueue.size();
  if (requestsInQueues > 0) processMemoryRequests();
}

void CommInterface::tryRead(MemSidePort *port) {
  MemoryRequest *readReq = port->readReq;
  Request::Flags flags;
  if (readReq->readLeft <= 0) {
    if (debug())
      DPRINTF(CommInterface,
              "Something went wrong. Shouldn't try to read if there aren't "
              "reads left\n");
    return;
  }
  int size;
...
void CommInterface::tryWrite(MemSidePort *port) {
  MemoryRequest *writeReq = port->writeReq;
  if (writeReq->writeLeft <= 0) {
    if (debug())
      DPRINTF(CommInterface,
              "Something went wrong. Shouldn't try to write if there aren't "
              "writes left\n");
    return;
  }
  ...
  std::memcpy(data,
              &(writeReq->buffer[writeReq->totalLength - writeReq->writeLeft]),
              size);
  RequestPtr req =
      make_shared<Request>(writeReq->currentWriteAddr, size, flags, masterId);
  ...
  if (debug())
    DPRINTF(
        CommInterface,
        "Trying to write to addr: 0x%016x, %d bytes, data 0x%08x through port: "
        "%s\n",
...
void CommInterface::tryRead(SPMPort *port) {
  MemoryRequest *readReq = port->readReq;
  Request::Flags flags;
  if (readReq->readLeft <= 0) {
    if (debug())
...
void CommInterface::tryWrite(SPMPort *port) {
  MemoryRequest *writeReq = port->writeReq;
  if (writeReq->writeLeft <= 0) {
    if (debug())
      DPRINTF(CommInterface,
...
void CommInterface::tryRead(RegPort *port) {
  MemoryRequest *readReq = port->readReq;
  Request::Flags flags;
  if (readReq->readLeft <= 0) {
    if (debug())
...
void CommInterface::tryWrite(RegPort *port) {
  MemoryRequest *writeReq = port->writeReq;
  if (writeReq->writeLeft <= 0) {
    if (debug())
      DPRINTF(CommInterface,
...
void CommInterface::enqueueRead(MemoryRequest *req) {
...
void CommInterface::enqueueWrite(MemoryRequest *req) {
  if (inRegRange(req->getAddress())) {
    // We want to immediately handle register requests
    // and bypass memory queues
    auto regport = getValidRegPort(req->getAddress());
...
Tick CommInterface::read(PacketPtr pkt) {
  if (debug())
    DPRINTF(DeviceMMR,
...
Tick CommInterface::write(PacketPtr pkt) {
  if (debug())
    DPRINTF(DeviceMMR,
  ...
  if (((*mmreg & 0x04) == 0x00) && int_flag) {
    if (int_num > 0) gic->clearInt(int_num);
    int_flag = false;
  }
  if (!tickEvent.scheduled()) {
    schedule(tickEvent, nextCycle());
...
uint64_t CommInterface::getGlobalVar(unsigned offset, unsigned size) {
  if (use_premap_data) {
...
Port &CommInterface::getPort(const std::string &if_name, PortID idx) {
  if (if_name == "local") {
...
MemoryRequest *CommInterface::findMemRequest(PacketPtr pkt, bool isRead) {
  if (isRead) {
...
void CommInterface::clearMemRequest(MemoryRequest *req, bool isRead) {
  if (isRead) {
    for (auto it = accRdQ.begin(); it != accRdQ.end(); ++it) {
      if ((*it) == req) {
        it = accRdQ.erase(it);
