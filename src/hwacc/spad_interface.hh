...
class SpadInterface {
 public:
    SpadInterface(gem5::Addr start, size_t spad_limit_, gem5::Addr tape_ptr): start_(start), end_(start), head_(start), spad_limit_(spad_limit_), tape_ptr_(tape_ptr), phase_(FWD) {}
    ...
    MemoryRequest *CreateReadRequest(std::shared_ptr<SALAM::Instruction> inst) const;

    MemoryRequest *CreateWriteRequest(std::shared_ptr<SALAM::Instruction> inst) const;
