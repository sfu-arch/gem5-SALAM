...
bool StreamBuffer::canReadStream(size_t len) {
  if (buffer.size() >= len) {
    return true;
  } else {
    return false;
...
bool StreamBuffer::canWriteStream(size_t len) {
...
bool StreamBuffer::tryReadStream(uint8_t *dst, size_t len) {
  if (buffer.size() >= len) {
    buffer.read(dst, len);
    return true;
  } else {
...
bool StreamBuffer::tryWriteStream(uint8_t *src, size_t len) {
  if ((buffer.size() + len) <= fifoSize) {
...
void StreamBuffer::readStream(uint8_t *dst, size_t len) {
  const bool success(tryReadStream(dst, len));
  panic_if(!success, "Buffer underrun in StreamBuffer::readStream()\n");
}

void StreamBuffer::writeStream(uint8_t *src, size_t len) {
...
Tick StreamBuffer::streamRead(PacketPtr pkt) {
  DPRINTF(StreamBuffer,
          "A read request of size %d was received by this stream buffer\n",
          pkt->getSize());
...
Tick StreamBuffer::streamWrite(PacketPtr pkt) {
  DPRINTF(StreamBuffer,
          "A write request of size %d was received by this stream buffer\n",
          pkt->getSize());
...
Tick StreamBuffer::status(PacketPtr pkt, bool readStatus) {
  // Provide a means of reading the current buffer capacity of the stream
  // Writes to this register do nothing
  if (pkt->isRead()) {
    DPRINTF(StreamBuffer,
            "The status of the buffer has been read. Current capacity is %d of "
            "%d bytes\n",
...
AddrRangeList StreamBuffer::getStreamAddrRanges() const {
  assert(streamSize != 0);
...
AddrRangeList StreamBuffer::getStatusAddrRanges() const {
  assert(statusSize != 0);
...
Port &StreamBuffer::getPort(const std::string &if_name, PortID idx) {
...
void StreamBuffer::serialize(CheckpointOut &cp) const {
  SERIALIZE_CONTAINER(buffer);
}

void StreamBuffer::unserialize(CheckpointIn &cp) {
